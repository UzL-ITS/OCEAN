...
def all_function_authors_from_ast(directory = "linux/", function_name_files = ["function_names_linux_kernel.json"], target_file = "all_authors_from_ast_wo_tests_with_code_linux_kernel.json"):
    ...
    for file_ending in file_endings:
        ...
        if len(files_without_tests) > best_ending[0]:
            best_ending = (len(files_without_tests), file_ending, files_without_tests.copy())
    print(f"Found {best_ending[0]} files with ending {best_ending[1]}")

    Language.build_library(
        # Store the library in the `build` directory
        'build/my-languages.so',
        ["/src/tree-sitter-c", "/src/tree-sitter-cpp"]
    )
    ...
    for function_name_file in function_name_files:
        with open(function_name_file) as fp:
            function_names_json = json.load(fp)
        function_names = [function_name["name"].strip() for function_name in function_names_json]

    with open(target_file, "w") as target_fp:
        target_fp.write("[")
        for file in best_ending[2]:
